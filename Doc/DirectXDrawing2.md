...
### La géosphère
Comme on peut le voir sur l'image concernant la sphère, tous les triangles n'ont pas une aire égale. Une géosphère approxime une sphère en utilisant des triangles avec des aires quasi égales et des longeurs de côtés similaires. 

![Géosphère](/Doc/Imgs/Geosphere.png)

Pour générer une géosphère, on commence par un icosaèdre (*icosahedron*), on subdivise chaque triangle et on projette les nouveaux sommets sur la sphère avec le rayon désiré. On peut répéter ce processus pour améliorer la tessellation. Voici un exemple d'implémentation : 
```c++
